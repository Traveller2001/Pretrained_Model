    ...
    for _ in train_iterator: # 遍历每个训练轮次
        ...
        for step, batch in enumerate(epoch_iterator): # 遍历训练集
            ...
            with autocast(enabled=bool(args.fp16)):
                ...
                for t in batch :
                    t.to(model_engine.local_rank) 
                batch_size, option_num, out = model_engine(batch)
            
                target = answer.view(-1, )
                target.to(model_engine.local_rank)
                # calculate loss
                ...
                model_engine.backward(scaler.scale(loss))

                if args.adv_type == 'fgm':
                    fgm.attack()  # 对抗训练
                    out = model_engine(batch)[2]
                    ...
                    model_engine.backward(adv_loss)
                    fgm.restore()
...
def main(args):
    ...
    for kFold_idx in range(args.k_folds_num):
        ...
        if args.do_train:
            ...
            eval_result = evaluate(args, model, None)
            output_dir = args.output_dir
            if not os.path.exists(output_dir):
                os.makedirs(output_dir)
            current_acc = eval_result['eval_accuracy']
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output_dir",
        default=SUPER_ROOT+'/model_zerobs12',
        type=str,
    ...
    parser.add_argument("--do_eval_during_train",default=False,action="store_true", help="Run evaluation during training at each logging step.",)
    ...
    parser.add_argument(
        ...
        type=int,default=24,help="Number of updates steps to accumulate before performing a backward/update pass.",)
